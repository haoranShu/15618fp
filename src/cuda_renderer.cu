...
////////////////////////////////////////////////////////////////////////////////
bool check_quadtree(const Quadtree_node *nodes, int idx, Points *pts, Parameters params)
{
    ...
    if (!(params.depth == params.max_depth || num_points <= params.min_points_per_node))
    {
        ...
        return check_quadtree(&nodes[params.num_nodes_at_this_level], 4*idx+0, pts, Parameters(params, true)) &&
            check_quadtree(&nodes[params.num_nodes_at_this_level], 4*idx+1, pts, Parameters(params, true)) &&
            check_quadtree(&nodes[params.num_nodes_at_this_level], 4*idx+2, pts, Parameters(params, true)) &&
            check_quadtree(&nodes[params.num_nodes_at_this_level], 4*idx+3, pts, Parameters(params, true));
    }
        ...
        Quadtree_node** nodes, Points** points)
{
    ...
    /*checkCudaErrors(cudaMalloc(&nodes, sizeof(Quadtree_node*) * 16));*/
    /*Quadtree_node ** ha = (Quadtree_node **)malloc(sizeof(Quadtree_node*) * 16);*/
    for (int i = 0; i < NUM_TREES; i++) {
        checkCudaErrors(cudaMalloc((void **)&nodes[i], max_nodes * sizeof(Quadtree_node)));
    }
    /*cudaMemcpy((void *)nodes, (void *)ha, sizeof(Quadtree_node*) * 16, cudaMemcpyHostToDevice);*/
    //checkCudaErrors(cudaMalloc((void **) nodes, max_nodes*sizeof(Quadtree_node) * 16));
    // Allocate memory to store the tree.
    int accu = 0;
    int last = 0;
    for (int i = 0; i < NUM_TREES; i++) {
        accu += num_points / NUM_TREES;
        if (i == NUM_TREES - 1) accu = num_points;
        Quadtree_node root;
        root.set_range(last, accu);
        root.set_bounding_box(0, 0, width, height);
        //Quadtree_node *nodes;
        //checkCudaErrors(cudaMalloc((void **) nodes, max_nodes*sizeof(Quadtree_node)));
        checkCudaErrors(cudaMemcpy(nodes[i], &root, sizeof(Quadtree_node), cudaMemcpyHostToDevice));
        ...
        /*printf("i = %d\n", i);*/
        build_quadtree_kernel<NUM_THREADS_PER_BLOCK><<<1, NUM_THREADS_PER_BLOCK, smem_size>>>(nodes[i], *points, params);
    ...
    for(int i = 0; i < NUM_TREES; i++) {
        ...
        checkCudaErrors(cudaMemcpy(host_nodes, nodes[i], max_nodes *sizeof(Quadtree_node), cudaMemcpyDeviceToHost));

        // Validate the results.
        bool ok = false;
        if (host_nodes[0].points_begin() == last && host_nodes[0].points_end() == accu) 
        {
            Parameters p(max_depth, min_points_per_node);
            ok = check_quadtree(host_nodes, 0, &host_points, p);
        } else {
             printf("expected begin %d end %d, found %d %d\n", last, accu, host_nodes[0].points_begin(), host_nodes[0].points_end());
        }
        std::cout << "Quadtree " << i << ": " << (ok ? "OK" : "FAILED") << std::endl;
        last = accu;
    }
        ...
        float* stamp)
{
    ...
    //traverse<<<4, 1>>>(&nodes[params.num_nodes_at_this_level], 4*idx+0, buf, box, pts, Parameters(params, true),
    //    pt_x, pt_y, x_reso, y_reso, stamp);
}
